    ...
    LPSTR lpCmdLine, int nCmdShow )
{
    ...
    MSG msg;
     ...
     {
     MessageBox(NULL, "This example requires Windows 95.","Regres32", MB_OK );
     return (FALSE);
     }
...
BOOL NEAR PASCAL TestNotify(HWND hDlg, LPOFNOTIFY pofn)
{
	switch (pofn->hdr.code)
	{
		...
		case CDN_HELP:
			MessageBox(hDlg, "Got the Help button notify.", "Regression32", MB_OK);
			break;
		...
		case CDN_SHAREVIOLATION:
			...
			MessageBox(hDlg, "Got a sharing violation notify.", "Regression32", MB_OK);
			break;
	}
	...
	switch (uMsg)
	{
		...
		default:
			...
			else if (uMsg == cdmsgShareViolation)
			{
				...
				MessageBox(hDlg, "Got a sharing violation message.", "Regression32", MB_OK);
				break;
			}
			return FALSE;
	}
