...
//							Logit[j].result_upper=((data[1][i]-intercepthigh)/slopehigh)*Logit[j].dilution;
//							if(Logit[j].result_upper!=0)
//							  Logit[j].result_dev=abs((int)((Logit[j].result_upper/Logit[j].result)*100)-100);
							j++;
								break ;
				case IDC_RID :
							Logit[j].input=data[1][i];
...
//							Logit[j].result_upper=((data[1][i]*data[1][i]-intercepthigh)/slopehigh)*Logit[j].dilution;
//							if(Logit[j].result_upper!=0)
//							  Logit[j].result_dev=abs((int)((Logit[j].result_upper/Logit[j].result)*100)-100);
							j++;
								break ;
...
//							Logit[j].result_upper=exp((nop-intercepthigh)/slopehigh)*Logit[j].dilution;
//							if(Logit[j].result_upper!=0)
//							  Logit[j].result_dev=abs((int)((Logit[j].result_upper/Logit[j].result)*100)-100);
								}
							j++;
...
//	  				 rows_temp=rows;
				 ...
				 else
					 {
					  ...
					  if(YesNo==IDCANCEL){SendMessage (hwnd, WM_CLOSE, 0, 0L); break;}
					  if(YesNo==IDYES)
						{
 ...
 //						FreeProcInstance((FARPROC)lpfnDlgInput);
	...
	case WM_CLOSE:
		...
		if(!Yklijninvoer)
		  {
			char text[30],t1[10],LastName[30];
...
// FreeProcInstance((FARPROC)lpfnDlgInput);
 ...
 case IDC_POLYNOOM :
			...
			Logit[regel].result=Poly(Dresult);
			break ;
 case IDC_LINEAIR :
			Logit[regel].input=Dresult;
			Logit[regel].result=((Dresult-intercept)/slope)*Logit[regel].dilution;
//			Logit[regel].result_upper=((Dresult-intercepthigh)/slopehigh)*Logit[regel].dilution;
//			if(Logit[regel].result_upper!=0)
//			 Logit[regel].result_dev=abs((int)((Logit[regel].result_upper/Logit[regel].result)*100)-100);
						break ;
 case IDC_RID :
			Logit[regel].input=Dresult;
			Dresult*=Dresult;
...
//			Logit[regel].result_upper=exp((nop-intercepthigh)/slopehigh)*Logit[regel].dilution;
//			if(Logit[regel].result_upper!=0)
//			Logit[regel].result_dev=abs((int)((Logit[regel].result_upper/Logit[regel].result)*100)-100);
			break ;
	}
	if(regel==maxregel)
	 Logit[regel].pointer=++rows;
