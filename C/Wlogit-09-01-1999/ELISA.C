...
int read_elisa_file(HWND hwnd)
{
	int n,j,x,y,fout,indx,xx,yy,cmp,dummy;
	 ...
	 for (x=dummy;x<MAXBREED;x++)
	  {
		...
		for(j=1;j<indx;j++)
			...
			else		 cmp=stricmp(monsternieuw,monsterindex);
	...
	while(TRUE)
	 {
	  ...
	  if(!fout)
		...
		while(++x<MAXBREED)
			...
			else				/* Anders is het een concentratie v/d ijklijn */
				{
				strcpy(nop1,strtok(item[x]," "));
				pos[y][x].concyk=atof(nop1);
				pos[y][x].verdunning=1;
				sprintf(pos[y][x].monsternaam,"IJK");
				if(item[x][0] != '?')	sprintf(pos[y][x].monsternaam,"0%-5.5s",nop1);
				else					sprintf(pos[y][x].monsternaam,"~%-5.5s",nop1);
				}
