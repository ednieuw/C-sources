...
//	 if(!Autosave) 			Pic_output_file[0]=0;
	...
	hDCMeta = CreateMetaFile((LPSTR)"Graph.wmf");
	SetMapMode (hDCMeta, MM_ANISOTROPIC) ;
	GetClientRect (hwnd, &rect) ;
	SetWindowExt(hDCMeta,rect.right, rect.bottom);
	SetROP2 (hDCMeta, nDrawingMode) ;
	Teken_assen(hwnd,hDCMeta,&axcal);
	Teken_logit_data(hwnd,hDCMeta,&axcal);
	teken_graph(hwnd,hDCMeta,&axcal,axcal.X_As,axcal.Y_As);
