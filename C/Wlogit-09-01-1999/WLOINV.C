...
*/     }
    ...
    if(Specialkey||toets==13)VerwerkSkey(toets);
       ...
       PostMessage(hwndEdit,WM_KEYDOWN,NULL,NULL);
       PostMessage(hwndEdit,WM_CHAR,toets,NULL);
       PostMessage(hwndEdit,WM_KEYUP,NULL,NULL);
...
char getkey(HWND hwndkey)
{
 ...
 while(!i ) {
		...
		if(msg.message == WM_QUIT)  PostMessage(HWND_BROADCAST,WM_QUIT,NULL,NULL);
